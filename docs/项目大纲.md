...
3. 如果数据集长度大于等于 `batch_size` 时，开始训练。每次从数据集中拿出 `batch_size` 的 `feature_planes` ，调用 `policy_value_net(batch_feature_planes)` 产生 $\log{\boldsymbol{p}}$ 和 $v$，根据公式  $loss = (z-v)^2-\boldsymbol{\pi}^T \log \boldsymbol{p} + c||\theta||^2$ 进行误差的反向传播，由此完成一次参数 $\theta$ 的更新；
